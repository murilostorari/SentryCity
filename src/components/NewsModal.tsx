  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <motion.div 
        ...
        className={`w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col border ${isDarkMode ? 'bg-[#1E1E1E]/95 border-white/10 text-white' : 'bg-white/95 border-gray-200 text-gray-900'}`}
      >
        ...
        <div className={`p-4 border-b ${isDarkMode ? 'border-white/10' : 'border-gray-200'} flex items-center justify-between`}>
          ...
          <button 
            onClick={onClose} 
            className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-[#2A2A2A] text-[#888888] hover:text-white hover:bg-[#333333]' : 'bg-gray-100 text-gray-500 hover:text-black hover:bg-gray-200'}`}
          >
            <X size={16} />
          </button>
        </div>
